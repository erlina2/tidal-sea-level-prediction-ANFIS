...
class ANFIS:
    ...
    def layer4_consequent(self, norm_firing_strengths, x1, x2):
        ...
        outputs = []
        inputs = np.array([x1, x2, 1])  # Tambahkan 1 untuk bias
    ...
    def train(self, X_train, y_train, X_val, y_val, mean_std_data):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            # Update parameter konsekuen menggunakan Least Squares
            A = np.zeros((len(X_train), 12))  # 4 rules * 3 parameters
            for i in range(len(X_train)):
...
def main():
    ...
    y = data['target']  # Sesuaikan dengan nama kolom target Anda
    ...
    anfis = ANFIS(learning_rate=0.01, epochs=100)
    anfis.train(X_train, y_train, X_val, y_val, mean_std)
    
    # Evaluasi model
    metrics, y_pred = anfis.evaluate(X_test, y_test, mean_std)
    print("\nHasil Evaluasi:")
    for metric, value in metrics.items():
        print(f"{metric}: {value:.4f}")
    
    # Visualisasi hasil
    anfis.plot_training_history()
    anfis.plot_prediction_comparison(y_test, y_pred)
    anfis.plot_error_distribution(y_test, y_pred)
    ...
    results.to_csv('anfis_predictions.csv', index=False)
    print("\nHasil prediksi disimpan ke 'anfis_predictions.csv'")
